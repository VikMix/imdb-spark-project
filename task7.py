...
def task7(spark_session, tb_df):
  ...
  most_popular_df = most_popular_df.withColumn(c.decade, f.floor(f.col(c.tb_startYear) / 10).cast(t.IntegerType()))
  window = w.partitionBy(c.decade).orderBy(f.desc(c.tr_averageRating))
  most_popular_df = most_popular_df.withColumn(c.ten_most_popular, f.row_number()\
                                               .over(window)).where(f.col(c.ten_most_popular) <= 10)
